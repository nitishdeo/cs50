...
char* indexes(const char* path)
{
    ...
    char *ret_str = (char*) malloc((strlen(path) + 12)*sizeof(char));
    ...
    for (int i = 0; i < n; i++)
    {
        ...
        {
            strcat(ret_str,"/index.html");
            return ret_str;
        }
        else if(strcmp(name, "index.php") == 0)
        {
            strcat(ret_str,"/index.php");
            return ret_str;
        }
    }
    return NULL;
...
bool load(FILE* file, BYTE** content, size_t* length)
{
    ...
    // to store each byte read
    BYTE ch;
    
    // dynamically allocate memory in heap
    BYTE * data = (BYTE*) malloc(sizeof(BYTE));
    
    while(fread(&ch, sizeof(BYTE), 1, file) == 1)
    {
        data[index] = ch;
        index++;
        
        // reallocate every time a new BYTE is read
        data =(BYTE*) realloc(data, sizeof(BYTE)*(index+1));
    }
    
    *content = data; // content stores the address of address of first byte
    *length = index; // length stores the address of total bytes
...
const char* lookup(const char* path)
{
    if(path !=NULL)
    {
        ...
        // return string according to the extention
        if(strcasecmp(extention,".css") == 0)
        {
            return "text/css";
        }
        ...
        if(strcasecmp(extention,".ico") == 0)
        {
            return "image/x-icon";
        }
        if(strcasecmp(extention,".js") == 0)
        {
            return "text/javascript";
        } 
        if(strcasecmp(extention,".php") == 0)
        {
            return "text/x-php";
        }    
...
bool parse(const char* line, char* abs_path, char* query)
{
    strcpy(abs_path,"\0");
    strcpy(query,"\0");
    // checking the method of request(should be GET)
    if(strncmp(line, "GET " , 4) !=0)
    {
        // return with method not allowed error (405)
        error(405);
        return false;
    }
    // the request line should not contain a double quote
    if(strstr(line,"\"") !=NULL)
    {
        // bad request error
        error(400);
    ...
    // check for the first single space
    line = strchr(line,' ');
    line++;
    
    // check for errors in the request line
    
    if(line[0] != '/')
    {
        // if the request-target does not begin with a '/' return with error 501; 
        error(501);
        return false;
    }
    ...
    {
        abs_path[index] = line[index];
        index++;
    }
    
    abs_path[index] = '\0'; // nul terminating the abs_path string
    ...
    if(line[index] == '?')
    {
        ...
        while(line[index] != ' ')
        {
            query[q_index] = line[index];
            q_index++;
            index++;
        }
        query[q_index] = '\0'; // nul terminating the query string
    } 
    index++; // skip the single space

    // check fot http version
    char http_ver[10] = "HTTP/1.1";
    int j=0;
    while(http_ver[j] != '\0' && line[index]!='\0')
    {
        if(http_ver[j] != line[index])
        {
            printf("%c %c",line[index], http_ver[j]);
            error(505);
            return false;
        }
        j++;
